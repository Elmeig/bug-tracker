...
const Store = {
    ...
    addBug(versionId, listId, bug) {
        ...
        if (!l) return null;
        const b = { id: crypto.randomUUID(), ...bug, createdBy: Auth.user?.name || 'Anónimo', createdByUser: Auth.user?.username || '', comments: [], createdAt: Date.now(), updatedAt: Date.now() };
        l.bugs.push(b);
        this.save();
        return b;
    },
...
function renderBugList() {
    ...
    if (globalFilter) {
        ...
        const filterLabels = { swVersion: 'Versión SW', createdBy: 'Tester', assignee: 'Asignado', client: 'Cliente' };
        $('#current-list-title').textContent = `${filterLabels[globalFilter.type] || globalFilter.type}: ${globalFilter.value}`;

        // For assignee filter, match any of the comma-separated values
        let allBugs;
        if (globalFilter.type === 'assignee') {
